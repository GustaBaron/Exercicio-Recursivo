...
def sala_demonio():
    ...
    print(
        "Ele te encara com olhos flamejantes e diz: 'Escolha agora, humano: correr ou morrer?'"
    )
    print("O que você faz? (Correr, Lutar)")
    choice = input("> ").strip().lower()
    if choice == "correr":
        dead(
            "Ele consegue te pegar e o mata facilmente sem resistência, pois estava de costas."
        )
    elif choice == "lutar":
...
def sala_ogro():
    ...
    print(
        "Ele te cumprimenta e diz que, se você acertar a resposta de sua pergunta, ele o deixará passar. Caso contrário, seu fim será trágico."
    )
    print(
        "A pergunta é: 'O que é que sempre corre, mas nunca anda? Às vezes murmura, mas nunca fala? Possui um leito, mas nunca dorme? Nasce, mas nunca morre?'"
    )
    print("Qual sua escolha? (Vulcão, Rio, Relâmpago, Vento)")
    choice = input("> ").strip().lower()
    if choice == "rio":
        print(
            "O ogro sorri e diz: 'Resposta correta. Você pode escolher entre duas portas agora: Direita ou Esquerda.'"
        )
...
def sala_ceu():
    ...
    print(
        "Após um caminho árduo, você chega a um lugar estranhamente aconchegante, que lembra um local de paz e luz."
    )
    print("Lá, você encontra uma entidade que lhe faz uma pergunta.")
    print(
        "A pergunta é: 'O que é leve como uma pluma, mas nenhum homem pode segurá-lo por muito tempo?'"
    )
    print("Escolha sua resposta: (Respiracao, Pó, Sombra, Água)")
    choice = input("> ").strip().lower()
    if choice == "respiracao":
        print(
            "Resposta correta! A entidade sorri e uma porta se materializa à sua frente."
        )
        print("Ao entrar, você encontra uma lança divina sobre um altar.")
        print(
            "Ao tocar nela, você perde a consciência e acorda novamente na sala inicial."
        )
...
def sala_inferno():
    ...
    print(
        "Após um caminho árduo, você chega a um lugar aberto, escuro e muito quente, cercado por labaredas de fogo."
    )
    print("Lá, você encontra um ser humanoide que lhe faz uma pergunta.")
    print(
        "A pergunta é: 'O que é mais poderoso do que Deus, mais maligno do que o Diabo, os pobres têm e os ricos precisam?'"
    )
    print("Escolha sua resposta: (Nada, Dinheiro, Poder, Inveja)")
    choice = input("> ").strip().lower()
    if choice == "nada":
        print(
            "Resposta correta! A entidade se curva e aponta para uma porta com uma capa flamejante atrás dela."
        )
...
def sala_capa_flamejante():
    ...
    if choice == "tocar na capa":
        print(
            "Ao tocar na capa flamejante, você sente uma energia intensa e é transportado de volta para a sala inicial."
        )
...
def sala_demonio_final_inferno():
    ...
    print(
        "O ser humanoide que lhe deu a capa flamejante aparece, mas desta vez, ele é mais poderoso."
    )
    ...
    if choice == "enfrentar":
        ...
        dead(
            "Sua alma é consumida pelo ser, e você se torna parte do submundo para sempre."
        )
    elif choice == "subjugar":
        print("Você decide usar o poder da capa flamejante para subjugá-lo.")
        print(
            "No entanto, ao subjugá-lo, você sente que o poder da capa está corroendo sua própria essência."
        )
        print(
            "Você derrota o ser, mas percebe que se tornou um novo governante do inferno."
        )
        dead(
            "Você venceu, mas ao custo de sua própria alma. Agora, você é o novo senhor das trevas."
        )
    elif choice == "usar a capa":
        print(
            "Você veste a capa flamejante, sentindo uma energia intensa que protege e fortalece seu corpo."
        )
        print(
            "Com a capa, você consegue enfrentar o ser humanoide e, após uma batalha épica, derrota-o completamente."
        )
        print(
            "O trono de ossos e chamas desmorona, e uma porta com a inscrição 'Liberdade' aparece."
        )
        print(
            "Você caminha até a porta e a atravessa, sentindo que finalmente escapou do inferno."
        )
...
def sala_demonio_final_ceu():
    ...
    print(
        "Desta vez, a lança que você encontrou começa a vibrar em sua mão, ansiosa para atacar."
    )
    print(
        "Você tem duas escolhas: correr, lutar ou subjulgar o demônio. O que você faz? (Correr, Lutar, Subjugar)"
    )
    choice = input("> ").strip().lower()
    if choice == "correr":
        dead(
        ...
        print(
            "Você decide subjulgar o demônio, usando a lança divina para controlar seu poder."
        )
        print(
            "A lança brilha intensamente e você ganha um enorme poder, subjugando o demônio."
        )
...
def sala_ceu_final():
    ...
    print(
        "O ser angelical explica que você deve enfrentar uma prova final usando a lança divina."
    )
    ...
    if choice == "bem":
        print(
            "Você usa a lança divina para combater as forças malignas que ameaçam o reino celestial."
        )
        print("Sua coragem e justiça são recompensadas, e o reino é restaurado à paz.")
        print(
            "Uma porta dourada se abre, levando você a um lugar de eterna felicidade e realização."
        )
        print("Parabéns! Você venceu o jogo e conquistou a paz eterna no céu!")
        exit_game()
    elif choice == "mal":
        print(
            "Você usa a lança divina para enfrentar as forças do bem, buscando poder e dominação."
        )
        print(
            "No entanto, seu coração é corrompido e você acaba traindo seus próprios princípios."
        )
        print(
            "O reino celestial se desmorona ao seu redor, e você é banido para um destino sombrio."
        )
...
def sala_demonio_final_ceu():
    ...
    print(
        "Você tem três escolhas: correr, lutar ou subjulgar o demônio. O que você faz? (Correr, Lutar, Subjugar)"
    )
    choice = input("> ").strip().lower()
    if choice == "correr":
        dead(
        ...
        print(
            "A lança brilha intensamente, e você usa seu poder para enfrentar o demônio."
        )
        ...
        print(
            "O demônio se submete à sua vontade, e uma nova porta aparece com a inscrição 'Saída'."
        )
